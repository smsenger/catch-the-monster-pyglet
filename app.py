...
lives_label = pyglet.text.Label(text="Lives 3", x=15, y=55, bold='True', batch=main_batch)
...
def update(dt):
    ...
    if is_drawing:
        ...
        for i in range(len(game_objects)):
            ...
            # for i in range(len(game_objects)):
            #     for k in range(i + 1, len(game_objects)):
            #         obj_1 = game_objects[i]

                    if not obj_1.dead and not obj_3.dead:
                        if obj_3.collides_with(obj_1):
                            lives -= 1
                            lives_label.text = f"{lives} lives remaining"
                            if lives == 0:
